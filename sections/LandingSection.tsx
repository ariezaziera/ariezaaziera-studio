...
export default function LandingSection({ setActivePage }: LandingSectionProps) {
  ...
  return (
    <section
      id="home"
      style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 40px", position: "relative", overflow: "hidden" }}
    >
      {/* Grid background */}
      <div style={{
      ...
      {/* Yellow glow */}
      <div style={{ position: "absolute", top: "40%", left: "20%", width: 400, height: 400, background: `radial-gradient(circle, ${YELLOW}18 0%, transparent 70%)`, pointerEvents: "none" }} />
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 900, margin: "0 auto", width: "100%" }}>
        ...
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 800, fontSize: "clamp(42px, 8vw, 88px)", lineHeight: 1.0, margin: "0 0 32px", letterSpacing: -2 }}>
          {lines.map((line, i) => (
        ...
        <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#666", maxWidth: 480, lineHeight: 1.8, marginBottom: 48, opacity: 0, animation: "fadeUp 0.6s 0.8s forwards" }}>
          I build interactive web systems that turn ideas into real products. Not just UI — full product thinking, front to back.
        </p>

        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", opacity: 0, animation: "fadeUp 0.6s 1s forwards" }}>
          <button
